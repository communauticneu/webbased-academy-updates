...
function install(doc){
  ...
  const syncContextualTextFont=()=>{
    ...
    layer.querySelectorAll?.('.academy-board-object-text[data-object-id]')?.forEach?.(node=>{
      if(boardActive){
        ...
        if(node.classList.contains('academy-text-heading'))node.style.setProperty('font-size','39px','important');
        else if(node.classList.contains('academy-text-normal'))node.style.setProperty('font-size','29px','important');
        else if(node.classList.contains('academy-text-small'))node.style.setProperty('font-size','23px','important');
  ...
  const syncTextFrameHeight=node=>{
    ...
    const height=Math.max(1,Math.ceil(Number(text.scrollHeight)||0)+6);
    if(height>6)node.style.setProperty('height',`${height}px`,'important');
    return true;
  };
