...
function install(doc){
  ...
  if(!doc.getElementById('academyDirectTextUxStyle')){
    ...
    style.textContent='.academy-board-content-field,.academy-board-quick-actions{display:none!important}.academy-board-object-text span[contenteditable="true"]{cursor:text;user-select:text;white-space:pre-wrap;outline:none}.academy-board-object-delete{position:absolute;right:-13px;top:-13px;width:24px;height:24px;border:0;border-radius:50%;display:grid;place-items:center;background:#b92f3b;color:#fff;font:700 14px/1 Arial,sans-serif;cursor:pointer;z-index:8;box-shadow:0 2px 7px rgba(0,0,0,.35)}';
  ...
  layer.addEventListener('pointerdown',event=>{
    ...
    if(node&&!event.target?.closest?.('[data-resize-handle]')&&!event.target?.closest?.('[data-direct-delete]'))selectVisual(node);
  },true);
...
root.AcademyPresentationTextDirectUx={install};
if(root.document){
