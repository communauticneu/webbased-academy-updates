...
test('Academy chalk surface uses the supplied 4K texture without enlargement and reaches behind avatar',()=>{
  const css=read('src/presentation-stage-v16.17.css');
  ...
  assert.match(css,/presentation-chalkboard\[data-size="large"\][\s\S]*?top:\s*0!important[\s\S]*?bottom:\s*0!important[\s\S]*?width:\s*88%!important[\s\S]*?height:\s*100%!important/);
